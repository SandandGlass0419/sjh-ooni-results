...
둘다 매일 측정할 수 있으면 좋겠지만 그러면 api 서버에서 데이터를 가져오기 힘들다.

## `Managed`랑 `Unmanaged`는 뭐가 다른가요?
`Managed`의 데이터는 mdm이 설치된 기기에서 측정한 결과이다.
mdm이 지정된 dns를 사용하도록 프로그래밍 되었다고 추측하기 때문에 측정 결과를 분리했다.
...
## 각 결과가 가지는 의미는 뭔가요?
- `ok`: 아무런 제한의 흔적이 없음
- `dns`: "dns tampering"으로 불리며, ooni 서버에서 측정한 dns 처리 결과와 네트워크에서 측정된 처리 결과를 비교할때 이상이(anomaly) 발견됨.
