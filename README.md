...
`DNS`는 안드로이드의 DNS서버 설정을 수동으로 설정하고, [dns.google](https://dns.google/) 서버를 사용해 측정한 것이다. 4월부터 측정했다.
...
- `confirmed`: 접근 제한이 확실히 확인됨. (주로 [warning.or.gov](https://warning.or.gov/) 처럼 잘 알려진 접근 거부 페이지를 감지)
